...
export class StaffAbsentRequestRepository extends BaseRepository<IStaffAbsentRequest> {
  ...
  async findByStaffId(staffId: string) {
    return this.rm.find({ staff_id: staffId }).sort({ createdAt: -1 }).exec();
  }

  async findPending() {
    return this.rm.find({ request_status: 'pending' }).exec();
  }

  async findByStatus(status: string) {
    return this.rm.find({ request_status: status }).exec();
  }

  async findStaffOff(from?: Date, to?: Date) {
    const filter: any = { request_status: RequestStatus.APPROVED };
    if (from && to) {
  ...
  async isOverlapping(staffId: string, from: Date, to: Date) {
    // exists() trong BaseRepository dùng rm
    return this.exists({
      staff_id: staffId,
  ...
  async findAbsentStaffByStaffId(staffId: string) {
    return this.rm
      .find({ staff_id: staffId, request_status: 'approved' })
      .sort({ from_date: -1 })
      .exec();
  }
}
