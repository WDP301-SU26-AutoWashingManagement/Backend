...
export class VehicleController {
  ...
  lookupClass = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const data = await this.service.findClassById(req.params.id);

        return sendSuccess(
            res,
            data,
            'Thông tin loại xe đã được lấy thành công',
        );
    } catch (error) {
        next(error);
    }
